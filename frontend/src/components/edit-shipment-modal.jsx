...
//       remarksWarehouse: 'Remarks (warehouse)',
//       appointmentShootedDate:"Appointment Shooted Date",
//       appointmentRequestedDate:"Appointment Requested Date",
...
const getFieldLabel = (key) => {
  if(!key) return "";
  const label = shipmentStatusDataType.find(item => item.fieldName === key)
  return label?.label ??  key.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase());
}
