  ...
  try {
     const [rows] = await sequelize.query(`
      SELECT
        ...
        GROUP_CONCAT(DISTINCT facility) AS facility,
    ...
    const filters = {
      ...
      channel: parse(rows[0]?.channel),
      facility: parse(rows[0]?.facility),
      location: parse(rows[0]?.location),
      statusPlanning: parse(rows[0]?.statusPlanning),
      statusWarehouse: parse(rows[0]?.statusWarehouse),
      statusLogistics: parse(rows[0]?.statusLogistics),
    };
